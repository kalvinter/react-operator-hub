...
export default function OutputChart() {
    ...
    if (gameData.overProduction || gameData.underProduction) {
        deltaComponent = (
            <span>
                {delta.deltaLabel} {gameData.productionDemandDelta.toFixed(0)} Watt
            </span>
        )
    } else {
        deltaComponent = <span>Perfect Production / Demand Match</span>
    }
    ...
    return (
        <Card className="w-full my-0">
            ...
            <div
                className={`w-full border-solid border-2 rounded border-gray-900 my-1 flex justify-between px-2 py-1 items-center ${delta.deltaBg}`}
            >
                {deltaComponent}
            </div>
        </Card>
